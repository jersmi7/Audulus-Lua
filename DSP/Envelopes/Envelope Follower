-- Control rate envelope follower with envelope attack and release
-- Jerry Smith :: 2023
...
function envelope(input, frames, sampleRate, attackTime, releaseTime)
    ...
    -- The absolute value of the input for audio signal
    ...
    local rate = absInput > prev_input and attackRate or releaseRate
    
    local envOut = (1 - rate) * prev_input + rate * absInput
